...
class TestS3StorageService extends FlatSpec with Matchers {
    ...
    it should "test for s3 storage" in {
        ...
        println("signed path to upload from external client", s3Service.getSignedURL("ekstep-dev-data-store", "testUpload/test-data-public1.log", Option(600), Option("w")))
        ...
        val objData = s3Service.getObjectData("ekstep-dev-data-store", "testUpload/test-blob.log")
